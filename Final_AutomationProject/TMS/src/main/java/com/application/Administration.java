...
public class Administration extends BaseAction {
    private String formName="Administration";
    ...
    public void UserManagement(ConfigTestRunner configTestRunner) {
        configTestRunner.setChildTest(configTestRunner.getParentTest().createNode("User Management Menu Selection Verification"));
            if(fnWaitForVisibility(getWebElement("HomePage","hamburgMenu",configTestRunner), Constants.AJAX_TIMEOUT)){
                ...
                WebElement userManagement=getSpanWithText(configTestRunner,"Administration");
                if (fnWaitForVisibility(userManagement, Constants.AJAX_TIMEOUT)) {
                    waitAndClick(userManagement,Constants.AJAX_TIMEOUT);
                    configTestRunner.getChildTest().log(Status.PASS,"User click on Administration menu.");
                    WebElement element = getSpanWithText(configTestRunner,"Users");
                    waitAndClick(element,Constants.AJAX_TIMEOUT);
                    configTestRunner.getChildTest().log(Status.PASS,"User click on Users menu");
                }
            }
        }
    ...
    public String fnCreateNewUser(ConfigTestRunner configTestRunner){
        ...
        try{
            ...
            getWebElement("HomePage","UserManagement_FName",configTestRunner).sendKeys(configTestRunner.getBaseAction().getTestData().get("FirstName"));
            getWebElement("HomePage","UserManagement_LName",configTestRunner).sendKeys(configTestRunner.getBaseAction().getTestData().get("LastName"));
